...
function add() {
    ...
    var AddressFormGroup = app.getForm('profile').object.address;
    var CountriesForBillingBoolean = true;
    var billingCountries = validateBillingCountry();
    AddressFormGroup.country.setOptions(billingCountries.iterator());
...
function handleForm() {
    ...
    addressForm.handleAction({
        ...
        create: function () {
            ...
            var address = addressBook.getAddress(session.forms.profile.address.addressid.value);
            if (!empty(address)) {
                session.forms.profile.address.addressid.invalidateFormElement();
                message = 'Form is invalid';
                success = false;
                return;
            } else if (!session.forms.profile.address.valid || !Address.create(session.forms.profile.address)) {
    ...
    if (message == 'Form is invalid' && !success){
        app.getView({
            Action: 'add',
            ContinueURL: URLUtils.https('Address-Form')
        }).render('account/addressbook/addressdetails');
        return;
    } else if (request.httpParameterMap.format.stringValue === 'ajax') {
...
function edit() {
    ...
    var billingCountries = validateBillingCountry();
    app.getForm(profileForm.address.country).setOptions(billingCountries.iterator());
    
    app.getForm(profileForm.address).copyFrom(address);
    if(profileForm.address.states != null && dw.system.Site.getCurrent().ID != 'ElizabethArdenUK' && dw.system.Site.getCurrent().ID != 'ElizabethArdenFR' && dw.system.Site.getCurrent().ID != 'ElizabethArdenES' && dw.system.Site.getCurrent().ID != 'ElizabethArdenDE' && dw.system.Site.getCurrent().ID != 'ElizabethArdenIT')
...
function setDefault() {
    ...
    session.privacy.eDDZipCode = addressBook.preferredAddress.postalCode;
...
function validateBillingCountry() {
	var CountriesForBillingBoolean = true;
	var script = require('app_elizabetharden_core/cartridge/scripts/countrydefinition/SetCountryOptions');
    var scriptResult = script.countries({
    	CountriesForBilling : (CountriesForBillingBoolean != null) ? CountriesForBillingBoolean : true
    });
    
    var FilteredCountries = scriptResult.FilteredCountries;
    var DefaultValue = scriptResult.DefaultValue;
...
/**
 * GroupOne- address validation integrations
 */
function validateDAV() {
    var result = {};
    var pdict;
    if (dw.system.Site.getCurrent().getCustomPreferenceValue('DAVService').toString() === 'GROUP_ONE') {
        pdict = dw.system.Pipeline.execute('GroupOne-ValidateProfileAddress');
        result.endNodeName = pdict.EndNodeName;
        result.params = {
            ContinueURL: URLUtils.https('Address-List'),
            ErrorFlag: pdict.ErrorFlag,
            ErrorDescription: pdict.ErrorDescription
        };
    }
