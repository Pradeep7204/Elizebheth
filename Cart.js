...
function continueShopping() {
	...
	/* OnClick Continue Shopping in Cart Page, Should go to Home Page */
...
function show() {
	...
	var pageMeta, cartShowAsset, Content;
    ...
    delete session.custom.creditCardNumber;
    
    //Delete brandCode custom attribute 
    delete session.custom.brandCode;

    cartForm.get('shipments').invalidate();
    
    //EAST-265 - apple pay: Apple pay address details getting populated in checkout shipping input fields
    if(!empty(request.httpParameterMap.applepayerror.value) && request.httpParameterMap.applepayerror.value){
	    Transaction.wrap(function () {
	        var cart  = app.getModel('Cart').get();
	    	var defaultShipment = cart.object.getDefaultShipment();
    ...
    cartShowAsset = Content.get('cart-show');

    pageMeta = require('~/cartridge/scripts/meta');
    if (cartShowAsset != null){
    	pageMeta.update(cartShowAsset);
    }
...
function submitForm() {
    ...
    var cart, formResult, cartForm, cartAsset, pageMeta, triggeredAction=null;
    ...
    formResult = cartForm.handleAction({
        ...
        'deleteCoupon': function (formgroup) {
            Transaction.wrap(function () {
                ...
                delete session.privacy.appliedCoupon;
            });
        ...
        'deleteProduct': function (formgroup) {
            Transaction.wrap(function () {
                ...
                dw.system.Pipeline.execute('Subscriptions-UpdateBasketValues');
                cart.calculate();
            });
            if (dw.system.Site.getCurrent().getCustomPreferenceValue('AO_Activate')) {
            	Pipeline.execute('AgilOne-RemoveProductFromBasket');
            	triggeredAction = 'deleteProduct';
            }
            return {
                cart: cart
            };
    ...
    if (formResult) {
        ...
        if (formResult.dontRedirect) {
        	...
        	if(!empty(triggeredAction)){
        		response.redirect(URLUtils.https('Cart-Show', 'triggeredAction',triggeredAction));
        	}else{
...
function addAllDTSToCart() {
    var cart = app.getModel('Cart').goc();
    var renderInfo = cart.addProductsToCart();
            ...
            if (product.isBundle()) {
                ...
                for (j = 0; j < childPids.length; j += 1) {
                    ...
                    if (childProduct) {
                        ...
                        var UpdateProductOptionSelections = require('app_elizabetharden_core/cartridge/scripts/cart/UpdateProductOptionSelections');
...
function addCouponJson() {
    ...
    if(couponStatus.CouponStatus == 'APPLIED'){
    	session.privacy.appliedCoupon = couponCode;
    }
    //});
    try{
	    if(typeof couponStatus.CouponStatus === 'undefined'){
	        couponStatus.error = true;
	        couponStatus.CouponStatus = "NO_ACTIVE_PROMOTION";
	    }
    }
    catch(e){
            var msg=e.message;
    ...
    if (request.httpParameterMap.format.stringValue === 'ajax') {
        ...
        r.renderJSON({
            ...
            message: Resource.msgf('cart.' + couponStatus.CouponStatus, 'checkout', null, couponCode),
...
/**
 * Adds a gift wrap product to the cart.
 *
 *
 */
function giftBoxProduct() {
	var resetCart;
	Transaction.wrap(function () {
		var cart = app.getModel('Cart').goc();
		var script = require('app_elizabetharden_core/cartridge/scripts/checkout/GiftboxProduct');
		var scriptResult = script.updateGiftBoxProduct({
			Basket : app.getModel('Cart').goc().object,
			Giftbox : request.httpParameterMap.giftbox.booleanValue,
			//GiftMessage : request.httpParameterMap.giftmessage.stringValue,
	    	IsGift : request.httpParameterMap.isgift.booleanValue,
	    	Product : dw.system.Site.getCurrent().getCustomPreferenceValue('GiftWrapProductID'),
	    	singleShippingForm : app.getForm('singleshipping')
	    });
		
		resetCart = scriptResult.ResetCart;
		savedGiftAttributes();
		cart.calculate();
    });
	app.getView({
		resetCart : resetCart
    }).render('checkout/shipping/giftboxstat');
}

function savedGiftAttributes() {
	var script = require('app_elizabetharden_core/cartridge/scripts/checkout/SaveGiftAttributes');
    var scriptResult = script.saveAttributes({
    	GiftFlag : session.forms.singleshipping.shippingAddress.isGift.value,
    	GiftMessage : session.forms.singleshipping.shippingAddress.giftMessage.value,
    	GiftWrappingFlag : session.forms.singleshipping.shippingAddress.isGiftWrap.value,
    	LineItemCtnr : app.getModel('Cart').goc().object
    });
}
...
function removeGiftWrap() {
	Transaction.wrap(function () {
	    var cart = app.getModel('Cart').goc();
		var script = require('app_elizabetharden_core/cartridge/scripts/checkout/RemoveGiftBoxProduct');
		var scriptResult = script.removeGiftBoxProduct({
			Basket : app.getModel('Cart').goc().object,
	    	Product : dw.system.Site.getCurrent().getCustomPreferenceValue('GiftWrapProductID'),
	    	singleShippingForm : app.getForm('singleshipping')
	    });
...
exports.AddAllDTSToCart = guard.ensure(['post'], addAllDTSToCart);
...
/** Adds Gift product to cart.
 * @see {@link module:controllers/Cart~giftBoxProduct} */
exports.GiftboxProduct = guard.ensure(['post'], giftBoxProduct);
/** Remove Gift product to cart.
 * @see {@link module:controllers/Cart~removeGiftWrap} */
exports.RemoveGiftWrap = guard.ensure(['post'], removeGiftWrap);
